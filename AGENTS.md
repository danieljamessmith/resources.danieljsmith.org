...
| `public/tex/_shared/pack_preamble.tex` | Shared Further Maths QBT/Soln pack preamble; mirrored by `exam-question-perturbation/resources/pack_preamble.tex` for Overleaf uploads |
...
The shared preamble keeps the existing red contents page convention and owns the
pack-level helpers (`\marks`, `\questionitem`, `\djsQbtHeader`,
`\djsSolnHeader`, `\djsFrontMatter`, and the legacy colored-cancel `\cxl`
macro).
...
this convention. The shared preamble keeps the established red contents-page
style and does not include tutoring worksheet timing/student header machinery.
...
Overleaf projects produced by `exam-question-perturbation/solutions/make_pack.py`
use a flat `\input{preamble.tex}` and bundle that preamble into the project.
`deploy-from-overleaf.mjs` rewrites this to
...
| `scripts/lib/pack-preamble.mjs` | `scripts/lib/pack-preamble.test.mjs` | Further Maths pack path/depth helpers, inline-to-shared-preamble migration, Overleaf import normalization, convention checks |
| `scripts/compile-tex.mjs` | `scripts/compile-tex.test.mjs` | Further Maths scope resolution across both strands; compile file discovery skips `_shared` |
