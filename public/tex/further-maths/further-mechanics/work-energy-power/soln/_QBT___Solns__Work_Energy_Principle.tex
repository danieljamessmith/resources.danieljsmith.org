...
\djsSolnHeader{Work-Energy Principle}
...
\fill[black, yshift=5pt] (A) circle (\particleRadius);
...
\pic["$\alpha$", draw, angle radius=12mm, angle eccentricity=1.15] {angle=horiz--base--top};
\end{tikzpicture}
\end{figure}
...
\begin{scope}[shift={(Aleft)}, rotate=\ang]
  ...
  \node[font=\small, rotate=\ang] at ({0.5*\bw},{0.5*\bh}) {$3\,\mathrm{kg}$};
\end{scope}
...
\begin{scope}[shift={(A)}, rotate=\ang]
  ...
  \fill[black] (0.05,\particleRad) circle (\particleRad);

  \draw[-{Stealth[length=3mm]}, thick, black]
    (\ustart,\uoffset) -- ++(\ulen,0)
    node[midway, above, xshift=-8pt, yshift=5pt] {$U\,\text{ms}^{-1}$};
...
\pic["$45^\circ$", draw, angle radius=7mm, angle eccentricity=1.45]
  {angle=R--C--P};
...
\begin{scope}[shift={(Afoot)}, rotate=\ang]
  ...
  \node[rotate=\ang] at (Acentre) {$A$};
\end{scope}
\node at ($(Acentre)+(\ang+90:-0.6)$) {$8\,\mathrm{kg}$};
...
\node at (Bcentre) {$B$};
\node[right=12.5pt] at (Bcentre) {$2\,\mathrm{kg}$};
...
\draw[{Stealth[length=2.5mm]}-{Stealth[length=2.5mm]}, thick]
  (D0) -- (D1) node[midway, sloped, above] {$d\,\mathrm{m}$};
