...
\input{../_shared/tmua_solutions_preamble.tex}
...
Thus the complete set is $0<x<\frac12$ or $1<x<2$, so the correct option is \textbf{A}. \\[5pt]
...
Let $x$ be the $x$-coordinate of $P$. 

Since $P$ lies on both curves, their $y$-coordinates are equal:
\[
x^2=(x-2)^2+b
\]
...
For the first curve, \( \dfrac{\mathrm{d}y}{\mathrm{d}x} =2x\) and for the second curve \( \dfrac{\mathrm{d}y}{\mathrm{d}x} =2x-4\)
...
Expanding all four factors would produce a quartic and create unnecessary work. 

Instead, notice that the two pairs $(x+3)(x+4)$ and $(x-2)(x+9)$ both have $7x$ as their linear term. 
...
Then
\begin{align*}
(x+3)(x+4)&=x^2+7x+12\\[5pt]
            &=A+12
\end{align*}
and
\begin{align*}
    (x-2)(x+9)&=x^2+7x-18\\[5pt]
            &=A-18
\end{align*}
...
Since $x$ is positive, $A=x(x+7)>0$, so $A=-54$ is impossible. Hence $x^2+7x=60$, which is option \textbf{D}.
...
\begin{align*}
A &=\int_{-4}^{2}(x^3-3x+5)-(9x-11)\,\mathrm{d}x\\[5pt]
&=\int_{-4}^{2}(x^3-12x+16)\,\mathrm{d}x\\[5pt]
&=\left[\frac{x^4}{4}-6x^2+16x\right]_{-4}^{2}\\[5pt] 
&=12-(-96)\\[5pt]
&=108
\end{align*}
...
Calculating $N$ in full is inefficient. 

Instead, pair factors of $2$ and $5$ to produce powers of $10$.

Since $8^{15}=2^{45}$,
\begin{align*}
N&=2^{45}\times 5^{40}\\[5pt]
 &=2^5\times 2^{40}\times 5^{40}\\[5pt]
 &=32 \times (2\times 5)^{40}\\[5pt]
 &=32\times 10^{40}\\[5pt]
 &=3.2\times 10^{41}
\end{align*}
...
Subtracting these equations gives $18a=18$, so $a=1$. 

Hence \[21+3b=9\]

giving $b=-4$. 

Substitution into the first original equation then gives
\[
4-8+c=4
\]
...
Recall that a quadratic sequence is precisely one with constant second differences.

Here we are looking only at the terms in positions
\[
2,\ 5,\ 8,\ 11,\ \ldots
\]

These positions are equally spaced: each time, the term number increases by $3$. For any quadratic sequence, moving forward by the same fixed number of places each time produces another quadratic sequence.

In fact, if the original second difference is $d$, then moving three places at a time gives second difference
\[
3^2d=9d
\]

So the terms in positions $2,5,8,11,\ldots$ can themselves be treated as a quadratic sequence. 
...
The next first difference must therefore be
\[
27+18=45
\]
...
so the correct option is again \textbf{C}.
...
A circle tangent to both axes has its centre one radius from each axis. 

Therefore the centres of the large and small circles are respectively $C=(1,1)$ and $D=(r,r)$.

The horizontal and vertical distances between the centres are both $1-r$. 

Also, because the circles touch externally without overlapping, the distance between their centres is the sum of their radii, $1+r$.
...
The line joining the centres is therefore the hypotenuse of a right-angled triangle whose other two sides both have length $1-r$. 
...
\]
The smaller circle must have $0<r<1$. The root $3+2\sqrt{2}$ is greater than $1$, so it is rejected. 

Also, $2\sqrt{2}<3$ and $2\sqrt{2}>2$, so $0<3-2\sqrt{2}<1$.
...
\begin{align*}
    b_{n+1}&=\log_3a_{n+1}\\[5pt]
           &=\log_3\bigl(3(a_n)^2\bigr)\\[5pt]
           &= 1 + 2\log_3a_n \\[5pt]
           &=1+2b_n
\end{align*}
...
The usual geometric sequence formula for the $n$th term is $ar^{n-1}$ for the term in position $n$. 

Here $c_n$ is in position $n+1$ because the indexing begins at $n=0$. 
...
First recall when a fraction has a terminating decimal.
...
The reason is that a terminating decimal can be written using a denominator which is a power of $10$, and
\[
10^r=2^r5^r
\]
...
because the denominator $8=2^3$ can be turned into a power of $10$ by multiplying by a suitable power of $5$.

Now consider the number of digits after the decimal point.

A fraction in lowest terms has at most two digits after the decimal point exactly when its denominator divides
\[
100=2^2\times5^2
\]
Similarly, it has at most one digit after the decimal point exactly when its denominator divides
\[
10=2\times5
\]
Therefore, for the decimal to have \textbf{exactly two digits} in its shortest form, its reduced denominator must divide $100$ but must not divide $10$.

Now factorise the denominator in the question:
\[
...
\frac{n}{1260}
\]
its denominator must still be a divisor of $1260$.

So the reduced denominator must divide both $1260$ and $100$. Their common prime factors give
\[
\gcd(1260,100)=2^2\times5=20
\]
Hence the possible reduced denominators are the divisors of $20$:
\[
1,\ 2,\ 4,\ 5,\ 10,\ 20
\]
But
...
1,\ 2,\ 5,\ 10
\]
all divide $10$, so these would give at most one digit after the decimal point.

Thus the only possible reduced denominators are
\[
4
\qquad\text{or}\qquad
20
\]
We now count how many values of $n$ give each denominator.

For reduced denominator $4$, write
\[
\frac{n}{1260}=\frac{a}{4}
\]
where $\dfrac{a}{4}$ is in lowest terms.
...
n=315a
\]
To keep the reduced denominator equal to $4$, the numerator $a$ must have no common factor with $4$. Among
\[
1,2,3,4
\]
...
2
\)
successful values of $n$ with reduced denominator $4$.

For reduced denominator $20$, write
\[
\frac{n}{1260}=\frac{a}{20}
\]
in lowest terms.
...
n=63a
\]
Now $a$ must have no common factor with $20$. Since
\[
20=2^2\times5
\]
this means that $a$ must be divisible by neither $2$ nor $5$.

Among the integers from $1$ to $20$, these are
\[
1,\ 3,\ 7,\ 9,\ 11,\ 13,\ 17,\ 19
\]
So there are
...
8
\)
successful values of $n$ with reduced denominator $20$.

Hence the total number of successful choices is
\[
2+8=10
\]
There are $1260$ equally likely possible values of $n$, so the required probability is
\[
\frac{10}{1260}
=
\frac{1}{126}
...
The quadratic formula can instead be used to find the two roots explicitly. Since

\begin{align*}
x&=\frac{7\pm\sqrt{49-4}}{2}\\[5pt]
&=\frac{7\pm\sqrt{45}}{2}\\[5pt]
&=\frac{7\pm3\sqrt5}{2}
\end{align*}
we may take
\[
...
For each factor, subtracting or adding $1$ gives expressions with the same denominator of $2$, which therefore cancels within the fraction. 

Hence
\begin{align*}
\frac{p-1}{p+1}\times\frac{q-1}{q+1}
&=
\frac{5+3\sqrt5}{9+3\sqrt5}
\times
\frac{5-3\sqrt5}{9-3\sqrt5} \\[7pt]
&=
\frac{(5+3\sqrt5)(5-3\sqrt5)}
     {(9+3\sqrt5)(9-3\sqrt5)} \\[7pt]
&=
\frac{25-45}{81-45} \\[7pt]
&=
-\frac{20}{36} \\[7pt]
&=
-\frac{5}{9}
...
The distance between two vertices of a cube is determined by how they are joined:

\begin{itemize}
\item vertices joined by an edge are distance $1$
\item vertices joined by a face diagonal are distance $\sqrt2$
\item vertices joined by a space diagonal are distance $\sqrt3$
\end{itemize}

So every triangle formed by three vertices must have side lengths chosen from $1$, $\sqrt2$ and $\sqrt3$.

If a triangle contains a space diagonal, then its side lengths are
\[
1,\ \sqrt2,\ \sqrt3
\]
and this is a right-angled triangle because
\[
1^2+(\sqrt2)^2=(\sqrt3)^2
\]
...
% Right-angled triangle with side lengths 1, sqrt2, sqrt3
\coordinate (A) at (FBL);
\coordinate (B) at (RBR);
\coordinate (C) at (RTR);

\draw[thick, red!75!black] (A) -- (B)
    node[midway, sloped, above=3pt, text=red!75!black] {$\sqrt2$};
\draw[thick, red!75!black] (B) -- (C)
    node[midway, right=2pt, text=red!75!black] {$1$};
\draw[thick, red!75!black] (A) -- (C)
    node[midway, sloped, above=3pt, text=red!75!black] {$\sqrt3$};
...
If there is no space diagonal but the triangle contains an edge, then the third side must be a face diagonal, so its side lengths are
\[
1,\ 1,\ \sqrt2
\]
which is again right-angled because
\[
1^2+1^2=(\sqrt2)^2
\]
...
% Right-angled triangle with side lengths 1, 1, sqrt2
\coordinate (A) at (FBL);
\coordinate (B) at (FBR);
\coordinate (C) at (FTR);

\draw[thick, red!70!black] (A) -- (B)
    node[midway, below=3pt, text=red!70!black] {$1$};
\draw[thick, red!70!black] (B) -- (C)
    node[midway, right=1.5pt, text=red!70!black] {$1$};
\draw[thick, red!70!black] (A) -- (C)
    node[midway, sloped, below=3pt, text=red!70!black] {$\sqrt2$};

\foreach \P in {A,B,C}
    \fill[red!70!black] (\P) circle (1.5pt);

\node[below left=2pt] at (A) {$A$};
\node[below right=2pt] at (B) {$B$};
\node[right=4pt] at (C) {$C$};
\end{tikzpicture}
% end q000658 figure F2
\end{djsSolutionFigure}

Hence the only triangles which are \emph{not} right-angled are those with no edge and no space diagonal, so all three sides must be face diagonals. Such a triangle has side lengths
\[
\sqrt2,\ \sqrt2,\ \sqrt2
\]
so it is equilateral, not right-angled:

\begin{djsSolutionFigure}
% begin q000658 figure F3
\begin{tikzpicture}[
    scale=1.2,
    x={(1cm,0cm)},
    y={(0.38cm,0.28cm)},
...
\draw[thick, blue!70!black] (B) -- (C)
    node[midway, sloped, above=0.1pt, text=blue!70!black] {$\sqrt2$};
...
\end{tikzpicture}
% end q000658 figure F3
\end{djsSolutionFigure}

We now count these non-right-angled triangles.

Fix the upper and lower faces of the cube. For any chosen vertex $A$, there are exactly two vertices on the opposite horizontal face which are a face diagonal away from $A$, and those two vertices are themselves a face diagonal apart. So each choice of $A$ determines exactly one equilateral triangle of this type.

There are $8$ choices for $A$, and each equilateral triangle is counted exactly once in this way, since it has a unique vertex on one horizontal face and the other two vertices on the opposite horizontal face. So there are
\[
8
\]
non-right-angled triangles.

The total number of ways to choose three vertices from the $8$ vertices of the cube is
\[
\binom{8}{3}=56
\]

Therefore the probability of obtaining a right-angled triangle is
\[
...
The answer choices differ in whether the two boundary values $k=-4$ and $k=0$ are included. Testing these two values is enough to distinguish all six options.

First take $k=-4$. The equation becomes
\[
2\cos^2x+4\sin x-4=0
\]
Using $\cos^2x=1-\sin^2x$ gives
\[
2\sin^2x-4\sin x+2=0
\]
and hence
\[
2(\sin x-1)^2=0
...
\sin x=1
\]
which has exactly one solution in the required range:
\[
x=\frac{\pi}{2}
\]
Therefore $k=-4$ must be included. This eliminates options \textbf{A}, \textbf{C} and \textbf{E}.

Now take $k=0$. The equation becomes
\[
2\cos^2x=0
\]
so
...
\cos x=0
\]
This has two solutions in the required range:
\[
x=\frac{\pi}{2},\ \frac{3\pi}{2}
\]
Therefore $k=0$ must not be included. Of the remaining options \textbf{B}, \textbf{D} and \textbf{F}, this eliminates \textbf{D} and \textbf{F}.

The only remaining option is \textbf{B}.
...
A positive solution $u=r$ produces the two distinct real solutions $x=\sqrt r$ and $x=-\sqrt r$, whereas $u=0$ produces only the single solution $x=0$. 

Since there are exactly three distinct real solutions in $x$, the two relevant solutions in $u$ must therefore be $u=0$ and $u=r$ for some $r>0$.

Substituting $u=0$ gives $b=0$. 

The equation then factorises as
\[
u^2+au=u(u+a)=0
\]
...
\begin{align*}
    f(x)&=u^2+au\\[5pt]
    &=\left(u+\frac a2\right)^2-\frac{a^2}{4}
\end{align*}
...
Consequently, every non-zero root occurs with its reflection in the $y$-axis: if $x=c$ is a root, then $x=-c$ is also a root. 

An odd number of distinct roots is therefore possible only if $x=0$ is also a root. 
...
The squared term can equal zero because $-a/2>0$. 

The minimum is therefore $-a^2/4=-4$, giving $a=-4$. 

Together with $b=0$, this again gives \textbf{B}.
