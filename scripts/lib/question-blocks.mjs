 ...
 *   \end{djsSolution}
 *
 * The legacy opener `\begin{tcolorbox}[...title={\textbf{Solution}}...]`
 * (closed by `\end{tcolorbox}`) is accepted too; freshly imported Overleaf
 * packs still use it until `migrate-solution-boxes` has run.
 *
 * The QBT file is the source of truth for the statement; the soln file owns
 * the prelude + Solution box + any trailing whitespace/`\newpage`.
 */

import { findDocBoundaries } from './tex-utils.mjs';

const DELIM_RE = /^% ---- Question (\d+) ----$/;
/** Legacy Solution box opener: a tcolorbox titled `\textbf{Solution}`. */
export const LEGACY_SOLUTION_OPEN_RE = /\\begin\{tcolorbox\}.*title=\{\\textbf\{Solution\}\}/;
/** Current Solution box opener from the shared pack preamble. */
export const DJS_SOLUTION_OPEN_RE = /\\begin\{djsSolution\}/;
/** Either Solution box opener; marks the statement/solution boundary. */
export const SOLUTION_OPEN_RE = new RegExp(
  `${LEGACY_SOLUTION_OPEN_RE.source}|${DJS_SOLUTION_OPEN_RE.source}`,
);
...
export function parseDoc(text) {
  ...
  // inside the question statement AND inside the Solution box, so the
 ...
 *                                        the Solution box opener).
 * @property {number} solutionOpenIdx   - Index in `block.lines` of the
 *                                        Solution box opener.
 * @property {number} solutionCloseIdx  - Index in `block.lines` of the
 *                                        matching closer, or -1 if unclosed.
 */

/**
 * Returns the environment name that closes a Solution box opener line:
 * `djsSolution` for the current box, `tcolorbox` for the legacy one, or
 * `null` if the line is not a Solution box opener.
 *
 * @param {string} line
 * @returns {'djsSolution' | 'tcolorbox' | null}
 */
export function solutionBoxEnv(line) {
  if (DJS_SOLUTION_OPEN_RE.test(line)) return 'djsSolution';
  if (LEGACY_SOLUTION_OPEN_RE.test(line)) return 'tcolorbox';
  return null;
}

/**
 * Finds the line that closes the Solution box opened at `openIdx`. Nested
 * environments of the same name are balanced, so a legacy tcolorbox with an
 * inner tcolorbox still resolves to its own `\end{tcolorbox}`.
 *
 * @param {readonly string[]} lines
 * @param {number} openIdx
 * @returns {number} - Index of the closing line, or -1 if none.
 */
export function findSolutionClose(lines, openIdx) {
  const env = solutionBoxEnv(lines[openIdx] ?? '');
  if (!env) return -1;
 ...
 *
 * Returns `null` if the block has no Solution box (work-in-progress).
 *
...
export function splitSolnBlock(block) {
  ...
  for (let i = 1; i < lines.length; i++) {
    if (SOLUTION_OPEN_RE.test(lines[i])) {
      openIdx = i;
      break;
    }
  }
  ...
  let preludeStart = openIdx;
  // Skip blank lines immediately before the Solution box.
  ...
  return {
    ...
    solutionOpenIdx: openIdx,
    solutionCloseIdx: findSolutionClose(lines, openIdx),
  };
}

/**
 * Replaces the statement region of a soln block with `newStatement`. The
 * delimiter, prelude, Solution box, and trailer are preserved exactly.
 *
