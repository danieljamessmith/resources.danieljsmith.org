...
// ---------------------------------------------------------------------------
// djsSolution boxes (current) alongside legacy tcolorbox Solution boxes
// ---------------------------------------------------------------------------

describe('solutionBoxEnv', () => {
  it('maps each Solution opener to the environment that closes it', () => {
    expect(solutionBoxEnv('\\begin{djsSolution}')).toBe('djsSolution');
    expect(
      solutionBoxEnv('\\begin{tcolorbox}[colback=white, colframe=scarlet, title={\\textbf{Solution}}, breakable]'),
    ).toBe('tcolorbox');
  });

  it('returns null for non-Solution lines, including other tcolorboxes', () => {
    expect(solutionBoxEnv('\\begin{tcolorbox}[colback=white]')).toBeNull();
    expect(solutionBoxEnv('\\end{djsSolution}')).toBeNull();
    expect(solutionBoxEnv('Some text')).toBeNull();
  });
});

describe('findSolutionClose', () => {
  it('finds \\end{djsSolution} for a djsSolution opener', () => {
    const lines = ['\\begin{djsSolution}', 'Body.', '\\end{tcolorbox}', '\\end{djsSolution}', '\\newpage'];
    expect(findSolutionClose(lines, 0)).toBe(3);
  });

  it('finds \\end{tcolorbox} for a legacy opener, balancing nested tcolorboxes', () => {
    const lines = [
      '\\begin{tcolorbox}[title={\\textbf{Solution}}]',
    ...
    expect(findSolutionClose(lines, 0)).toBe(5);
  });

  it('returns -1 when the box is unclosed or the line is not an opener', () => {
    expect(findSolutionClose(['\\begin{djsSolution}', 'Body.'], 0)).toBe(-1);
    expect(findSolutionClose(['Body.', '\\end{djsSolution}'], 0)).toBe(-1);
  });
});

describe('splitSolnBlock with djsSolution', () => {
  const DJS_DOC = [
    '\\begin{document}',
    '\\begin{enumerate}',
    '% ---- Question 1 ----',
  ...
  it('splits at \\begin{djsSolution} and records the matching closer', () => {
    const block = parseDoc(DJS_DOC).blocks[0];
    const split = splitSolnBlock(block);
    expect(split).not.toBeNull();
    expect(split.statement).toEqual([
      '\\questionitem',
    ...
    expect(block.lines[split.solutionOpenIdx]).toBe('\\begin{djsSolution}');
    expect(block.lines[split.solutionCloseIdx]).toBe('\\end{djsSolution}');
  });

  it('records the legacy closer for a legacy Solution tcolorbox', () => {
    const block = parseDoc(SOLN_TWO_Q).blocks[0];
    const split = splitSolnBlock(block);
    expect(block.lines[split.solutionCloseIdx]).toBe('\\end{tcolorbox}');
  });

  it('syncs a djsSolution pack from QBT without touching the solution body', () => {
    const solnDjs = SOLN_TWO_Q.replace(/\\begin\{tcolorbox\}\[[^\n]*\]/g, '\\begin{djsSolution}').replace(
      /\\end\{tcolorbox\}/g,
      '\\end{djsSolution}',
    );
    const drifted = solnDjs.replace('First question statement.', 'Old wording.');
    const result = syncPair(QBT_TWO_Q, drifted);
    expect(result.fatal).toBe(false);
    expect(result.changes.map((c) => c.n)).toEqual([1]);
    expect(result.newSolnText).toBe(solnDjs);
  });
});
