 ...
 *
 *   scope    further-maths (default) | all
 *   --deploy  copy built PDFs to their sibling deployed location on success
 *
 * TMUA is never compiled here: its .tex and .pdf are managed files, written
 * together, and a second toolchain building those PDFs could ship a copy
 * that differs from the one committed with its source. `all` therefore
 * covers Further Maths only, and the `tmua` scope is refused.
 *
...
export const MANAGED_SCOPES = new Set(['tmua']);

export function resolveSearchRoot(scopeArg, texRootArg = texRoot) {
  if (MANAGED_SCOPES.has(scopeArg)) {
    throw new Error(
      `compile-tex: ${scopeArg} files are managed and are not compiled here`,
    );
  }
...
/** True for a path inside a managed scope, which compile-tex never builds. */
export function isManagedTexPath(relFromRepo) {
  const parts = relFromRepo.replace(/\\/g, '/').split('/');
  const texIndex = parts.indexOf('tex');
  return texIndex >= 0 && MANAGED_SCOPES.has(parts[texIndex + 1]);
}
...
export function collectCompileFiles(searchRootArg, repoRootArg = repoRoot) {
  return walkTexFiles(
    searchRootArg,
    repoRootArg,
    (rel) => shouldProcessTexFile(rel) && !isManagedTexPath(rel),
  ).sort();
}
...
function main() {
  let searchRoot;
  try {
    searchRoot = resolveSearchRoot(scope);
  } catch (e) {
