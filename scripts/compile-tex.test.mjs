...
import {
  ...
  isManagedTexPath,
  resolveSearchRoot,
} from './compile-tex.mjs';

describe('managed TMUA files', () => {
  it('refuses the tmua scope', () => {
    expect(() => resolveSearchRoot('tmua', 'TEXROOT')).toThrow(/managed/);
  });

  it('recognises paths under public/tex/tmua as managed', () => {
    expect(isManagedTexPath('public/tex/tmua/qbt/TMUA_SetA_Paper1.tex')).toBe(true);
    expect(isManagedTexPath('public\\tex\\tmua\\soln\\X.tex')).toBe(true);
    expect(
      isManagedTexPath('public/tex/further-maths/core-pure/tmua/qbt/_QBT__A.tex'),
    ).toBe(false);
  });

  it('never collects TMUA files, even under the all scope', () => {
    ...
    try {
      ...
      for (const rel of [
        ...
        'tmua/qbt/TMUA_SetA_Paper1.tex',
      ]) {
        const abs = join(texRoot, rel);
        mkdirSync(join(abs, '..'), { recursive: true });
        writeFileSync(abs, '');
      }
      const found = collectCompileFiles(texRoot, repoRoot).map((p) =>
        p.slice(repoRoot.length + 1).replace(/\\/g, '/'),
      );
      expect(found).toEqual(['public/tex/further-maths/core-pure/topic/qbt/_QBT__A.tex']);
    } finally {
